...
WFBorder {
   ...
   /* states: [
        ...
        State {
          ...
          PropertyChanges {
              target:thisWindow
              width:if(parent.width * 0.80 < maxiumWidth) {parent.width * 0.80} else {maxiumWidth}
              height:if(parent.height * 0.80 < maxiumHeight) {parent.height * 0.80} else {maxiumHeight}
              opacity: 1
          }
        },
    ...
    Column {
        ...
        anchors.centerIn: parent
        //visible:if(parent.height == thisWindow.height * 0.80) {true} else {false}
